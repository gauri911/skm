...
import 'package:usb_communication/src/usb_fido.dart'; // Updated import path
...
class PivProvider extends ChangeNotifier {
  ...
  Future<bool> verifyPin(String pin) async {
    try {
      ...
      final result = await _usbFido.verifyUserPin(pin);
  ...
  Future<bool> changePin(String oldPin, String newPin) async {
    try {
      ...
      final result = await _usbFido.changePin(oldPin, newPin);
      _updatePinStatus(result);
      return result >= 0;
    } catch (e) {
  ...
  Future<bool> changePuk(String oldPuk, String newPuk) async {
    try {
      final result = await _usbFido.changePuk(oldPuk, newPuk);
      return result >= 0;
    } catch (e) {
  ...
  Future<bool> changeManagementKey(String oldKey, String newKey) async {
    try {
      final result = await _usbFido.changeManagementKey(oldKey, newKey);
      return result >= 0;
    } catch (e) {
  ...
  Future<bool> resetToDefaultPin() async {
    try {
      final result = await _usbFido.resetToDefaultPin();
      if (result >= 0) {
  ...
  Future<int> getRemainingPinTries() async {
    try {
      final tries = await _usbFido.getRemainingPinTries();
